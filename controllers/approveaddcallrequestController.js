...
const updateApproveaddcallrequeststatus = (req, res) => {
  ...
  approveaddcallrequestModel.updateApproveaddcallrequeststatus(
    approveaddcallrequestId,
    status,
    (err, result) => {
      if (err) {
      ...
      // ✅ If status = Approved → update tbl_booking.callDisabled = NULL
      if (status === "Approved") {
        approveaddcallrequestModel.enableBookingByApproveRequest(
          approveaddcallrequestId,
          (err2) => {
            if (err2) {
              console.error("Error updating booking callDisabled:", err2);
              // Not blocking response, just logging
            }
          }
        );
      }
