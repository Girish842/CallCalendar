...
const getConsultantPresaleSettings = (req, res) => {
  const { consultantid } = req.body;

  dashboardModel.getConsultantPresaleSettingData(consultantid, (err, result) => {
    if (err) {
      console.error("Error fetching consultant setting:", err);
      return res.status(500).json({ status: false, message: "Server error" });
    }
...
const updatePresaleBookingSlots = async (req, res) => {
  try {
    ...
    // Prepare insert data (null if not provided)
    const data = {
      user_id: payload.user_id,
      sun_time: payload.sun_time ?? null,
      mon_time: payload.mon_time ?? null,
      tue_time: payload.tue_time ?? null,
      wed_time: payload.wed_time ?? null,
      thu_time: payload.thu_time ?? null,
      fri_time: payload.fri_time ?? null,
      sat_time: payload.sat_time ?? null,
    };

    // Call model
    dashboardModel.updatePresaleBookingSlots(data, (err, result) => {
      if (err) {
        console.error("Database error (updatePresaleBookingSlots):", err);
        return res
          .status(500)
          .json({ status: false, message: "Database operation failed" });
      }

      return res.status(200).json({
        status: true,
        message: "Pre-sale booking slots updated successfully",
      });
    });
  } catch (error) {
    console.error("Unexpected error (updatePresaleBookingSlots):", error);
