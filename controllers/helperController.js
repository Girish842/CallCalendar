...
const getSuperConsultants = (req, res) => {
  try {
    const { status } = req.body;

    helperModel.getSuperConsultants(status, (err, results) => {
      if (err) {
        console.error("Error fetching admin list:", err);
        return res
          .status(500)
...
const fetchBookingDetailsWithRcOld = (req, res) => {
...
const fetchBookingDetailsWithRc = (req, res) => {
  ...
  helperModel.getBookingDetailsWithRc(id, (err, bookingRow) => {
    ...
    // Step 1: Get consultant settings
    helperModel.getConsultantSettingData(consultantId, (err2, settingRow) => {
      if (err2) {
        console.error("Error fetching consultant setting:", err2);
        return res.status(500).json({
      ...
      // Step 2: Get presale slots (based on role)
      helperModel.getConsultantPresaleSlots(consultantId, (err3, presaleSlots) => {
        if (err3) {
          console.error("Error fetching consultant presale slots:", err3);
          return res.status(500).json({
            status: false,
            message: "Error fetching consultant presale slots",
          });
        }
        ...
        return res.json({
          ...
          bookingDetails: bookingRow,
          consultantSettings: settingRow || {},
          consultantPresaleSlots: presaleSlots || {},
        });
      });
    });
  });
      ...
      (err, exists) => {
        ...
        bookingModel.getAdminById(followerConsultantId, (err, follower) => {
          ...
          helperModel.insertFollower(followerData, (err, insertId) => {
            ...
            const comment = `${user.fld_admin_type} ${user.fld_name} added ${follower.fld_name
              ...
              helperModel.getFollowerByID(insertId, (err, fullFollowerRow) => {
                ...
                try {
                  if (follower.fld_email) {
                    ...
                    const message = `${formatType(user.fld_admin_type)} ${user.fld_name
                      } added you as a follower to the ${bookingRow.fld_sale_type
                      } call with client ${bookingRow.fld_name} - Booking id <b>${bookingRow.fld_bookingcode
                      }</b> <br/> 
  ...
  bookingModel.getBookingById(booking_id, (err, bookingRows) => {
    ...
    if (String(bookingInfo.fld_otp).trim() === String(str2).trim()) {
      ...
      bookingModel.updateBooking(booking_id, updateData, (err) => {
        ...
        const cancelConflictingBookings = () => {
          const { fld_booking_date, fld_booking_slot, fld_consultantid } = bookingInfo;

          const query = `
    SELECT id, fld_call_request_sts 
    FROM tbl_booking 
    WHERE fld_booking_date = ? 
      AND fld_booking_slot = ?
      AND fld_consultantid = ?
      AND id != ?`;

          db.query(query, [fld_booking_date, fld_booking_slot, fld_consultantid, booking_id], (err, rows) => {
            if (err) {
              console.error("Error fetching bookings for cancellation:", err);
              return;
            }

            if (!rows || rows.length === 0) return;

            const cancelStatuses = ["Call Scheduled", "Accept", "Rescheduled", "Call Rescheduled"];

            rows.forEach((row) => {
              if (cancelStatuses.includes(row.fld_call_request_sts)) {
                const updateQuery = `
          UPDATE tbl_booking 
          SET fld_call_request_sts = 'Cancelled' 
          WHERE id = ?`;

                db.query(updateQuery, [row.id], (updateErr) => {
                  if (updateErr) {
                    console.error(`Error cancelling booking ${row.id}:`, updateErr);
                  } else {
                    console.log(`Booking ${row.id} cancelled due to OTP confirmation`);
                  }
                });
              }
            });
