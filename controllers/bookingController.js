...
<body style="font-family: Arial, sans-serif;">
  ...
  <table style="border: 1px solid #e5e5e5; width: 100%; margin-top: 10px;">
    ...
    ${(booking.fld_call_confirmation_status ===
      "Call Confirmation Pending at Client End test test test")
      ? `
      <tr>
...
const handleAcceptedBookingNotification = (bookingInfo) => {
  try {
    ...
    // If initial mail already sent, send reschedule mail instead of OTP mail
    if (bookingInfo && bookingInfo.initialMailSent === 1) {
      const oldDateTime = `${bookingInfo.fld_booking_date_old} ${bookingInfo.fld_booking_slot_old}`;
      const newDateTime = `${bookingInfo.fld_booking_date} ${bookingInfo.fld_booking_slot}`;
      return sendRescheduleMail(
        { booking: bookingInfo, oldDateTime, newDateTime },
        (err) => {
          if (err) {
    ...
    console.log("otpPageUrl", otpPageUrl);
    ...
    sendPostmarkMail(
      ...
      (emailErr) => {
        ...
        // Mark initialMailSent when not already set to 1
        if (bookingInfo?.id) {
          bookingModel.setInitialMailSentIfNeeded(bookingInfo.id, (err) => {
            if (err) {
              console.error("Error updating initialMailSent:", err);
            }
          });
        }
      }
...
const updateReassignCallStatus = (req, res) => {
  ...
  bookingModel.checkConflictingBookings(
    ...
    (err, conflicts) => {
      ...
      const updateData = {
        fld_consultantid: consultant_id,
        fld_consultant_another_option: null,
      };
      ...
      bookingModel.updateBooking(bookingid, updateData, (err, success) => {
        if (err || !success) {
          return res.json({
            status: false,
            message: "Failed to update booking." + err,
          });
        }
